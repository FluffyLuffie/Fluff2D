...
namespace ImGui
{
    ...
    IMGUI_API bool          SliderBehaviorParam(const ImRect& bb, ImGuiID id, void* p_v, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb, const std::vector<float> &keyValues);
    ...
    template<typename T, typename SIGNED_T, typename FLOAT_T>   IMGUI_API bool  SliderBehaviorParamT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, T* v, T v_min, T v_max, const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb, const std::vector<float> &keyValues);
